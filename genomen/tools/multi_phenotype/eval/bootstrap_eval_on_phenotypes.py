...
def run_genomen(configs, phenotype, compute_local_shap):
    ...
    for i in range(BOOTSTRAP_N):
        ...
        logger.info(f"Geno-only mean score on test set: {np.mean(geno_scores):.4f} ± {np.std(geno_scores):.4f}")
        logger.info(f"Covar-only mean score on test set: {np.mean(covar_scores):.4f} ± {np.std(covar_scores):.4f}")
        logger.info(f"Combined mean score on test set: {np.mean(combined_scores):.4f} ± {np.std(combined_scores):.4f}")
